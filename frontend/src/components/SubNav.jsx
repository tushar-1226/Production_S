...
const SubNav = () => {
  ...
  return (
    <div className="lg:px-22 w-full h-16 px-5 flex items-center  justify-between absolute top-16 left-0 z-30 bg-white">
      ...
      <div className={`absolute top-full left-0 w-full bg-white shadow-lg overflow-hidden transition-all duration-500 ease-in-out lg:hidden flex flex-col z-40 ${isMenuOpen ? "h-[30vh] opacity-100" : "h-0 opacity-0"}`}>
        <div className='flex flex-col gap-3 p-5'>
          <a href="/ride" className='text-sm font-semibold'>Request a ride</a>
          <a href="/ride" className='text-sm font-semibold'>Reserve a ride</a>
          <a href="/ride" className='text-sm font-semibold'>See prices</a>
          <a href="/exploreride" className='text-sm font-semibold'>Explore ride options</a>
          <a href="/airport" className='text-sm font-semibold'>Airport rides</a>
        </div>
      </div>

      <div onClick={() => setIsMenuOpen(false)} className={`absolute top-full left-0 w-full h-screen bg-black/50 z-30 lg:hidden transition-opacity duration-500 ${isMenuOpen ? "opacity-100 visible" : "opacity-0 invisible"}`}></div>
    </div>
  )
}
